...
def tencent_translation(tc, context, language):
    for lan in language:
        text = tc.get_result(context, lan)
        time.sleep(0.21)
        res = tc.get_result(text)
        if len(language) != 1:
            time.sleep(0.21)
        yield  res
...
if __name__ == '__main__':
    tc = TencentCloud()
    langs = ['zh', 'en', 'jp', 'kr', 'de', 'fr', 'es', 'it', 'tr', 'ru', 'pt', 'vi', 'id', 'ms', 'th']
    for lang in langs:
        text = tc.get_result('下大雨了该', lang)
        time.sleep(0.21)
        print(text)
